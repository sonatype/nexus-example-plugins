...
package org.sonatype.nexus.examples.url.config;
...
import org.sonatype.nexus.examples.url.config.model.io.xpp3.UrlRealmConfigurationXpp3Reader;
import org.sonatype.nexus.examples.url.config.model.io.xpp3.UrlRealmConfigurationXpp3Writer;
...
/**
 * URL realm configuration, a special one, as it's configuration is "one way", is read only. Also, the model
...
{
  private static class UrlRealmModelReader
      ...
      implements Versioned
  {
    ...
    private final UrlRealmConfigurationXpp3Reader modelloReader = new UrlRealmConfigurationXpp3Reader();
  ...
  private final UrlRealmModelReader urlRealmModelReader;

  private Configuration configuration;

  @Inject
  public DefaultUrlRealmConfiguration(final ApplicationConfiguration applicationConfiguration,
                                      final SecuritySystem securitySystem) throws IOException
  {
    this.configurationFile = new File(applicationConfiguration.getConfigurationDirectory(), "url-realm.xml");
    this.securitySystem = securitySystem;
    this.urlRealmModelReader = new UrlRealmModelReader();
  }
  ...
  protected Configuration load() throws IOException {
    ...
    final Configuration result = ModelloUtils.load(Configuration.MODEL_VERSION, this.configurationFile,
        urlRealmModelReader, new ModelloModelUpgrader("1.0.0", Configuration.MODEL_VERSION)
    {
      @Override
      public void doUpgrade(final Reader reader, final Writer writer) throws IOException, XmlPullParserException {
        ...
        final Configuration conf = new UrlRealmConfigurationXpp3Reader().read(reader);
        conf.setVersion(Configuration.MODEL_VERSION);
        new UrlRealmConfigurationXpp3Writer().write(writer, conf);
      }
    });
    ...
    if (vr.isValid()) {
      ...
      log.warn("Invalid URL Realm configuration, not using it ", new InvalidConfigurationException(vr));
      return new Configuration();
    }
  }
  ...
  private ValidationResponse validateConfig(Configuration config) {
    ...
    if (StringUtils.isEmpty(config.getDefaultRole())) {
      ...
      try {
        securitySystem.getAuthorizationManager("default").getRole(config.getDefaultRole());
      }
      catch (Exception e) {
